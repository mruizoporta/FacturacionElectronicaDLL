/*
  vwFacturasDetalleXMLPOS — versión orientada a rendimiento.

  Cambios respecto a la vista típica con subconsultas correlacionadas:
  - Se eliminan las repeticiones de (SELECT par_itbisincluido FROM PARAMETROS ...).
  - Se usa un agregado por empresa (GROUP BY emp_codigo) para el flag ITBIS incluido.
  - Se mantiene la misma lógica de cálculos y ROW_NUMBER().

  Ajustar USE [casa] según tu base. Probar en desarrollo antes de producción.

  Índices sugeridos: docs/IX_vwFacturasDetalleXMLPOS_support.sql
*/
USE [casa];
GO
...
ALTER VIEW [dbo].[vwFacturasDetalleXMLPOS]
AS
SELECT
    usu_codigo,
    caja,
