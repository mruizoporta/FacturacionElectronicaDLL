## Archivo TXT – Tipo e-CF 33

Nota de Débito Electrónica (Tipo e‑CF 33).  
Resumen de campos para el archivo TXT según la pestaña **“Campos e‑CF 33”** de la matriz de LUGANIS.  
...
| 3       | `<RazonSocialComprador>`        | 150       | ALFA NUM | 3      |                                 |
...
- Esta hoja será la referencia para:
  - `LuganisXmlReader.Parse` para **tipo 33** (nota de débito).
  - `LuganisTxtBuilder.BuildTxtTipo33`.
- En particular, la sección `INFR` es clave para vincular la nota de débito con el e‑CF original (`NCFModificado`, `RNCOtroContribuyente`, `FechaNCFModificado`, etc.).
